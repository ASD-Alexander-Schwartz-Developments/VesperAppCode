...
namespace VesperApp.Controls
{
    public class ProgressStatus : INotifyPropertyChanged, IProgressStatus
    {
        ...
        /// <summary>Gets progress in percent shown by ProgressBar in ProgressDialog.</summary>
        public int ProgressPercent2
        {
            get => _progressPercent2;
            private set
            {
                _progressPercent2 = value;
        ...
        public void Update(string message, int progressPercent, int pp2)
        {
            Message = message;
            if (progressPercent >= 100 && pp2 >= 100)
            {
                IsFinished = true;
            }
